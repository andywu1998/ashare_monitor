...
MARKET_TOP_N="20"
MARKET_LOOKBACK="30"
MARKET_INSTITUTION_TOP_N="5"
MARKET_INSTITUTION_LOOKBACK_DAYS="365"
...
Usage:
  ...
  sh sync-run.sh --ashare-market-brief [--date latest|YYYYMMDD] [--message-id <message_id>]
...
Modes:
  ...
  --ashare-market-brief:
    Generate an A-share market brief from real Tushare data and upload it
    through the same Feishu docx + Bitable path as company reports.
EOF
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --lookback)
      if [ "$#" -lt 2 ]; then
        echo "Missing value for --lookback" >&2
        exit 1
      fi
      MARKET_LOOKBACK="$2"
      shift 2
    ...
    --institution-top-n)
      if [ "$#" -lt 2 ]; then
        echo "Missing value for --institution-top-n" >&2
        exit 1
      fi
      MARKET_INSTITUTION_TOP_N="$2"
      shift 2
      ;;
    --institution-lookback-days)
      if [ "$#" -lt 2 ]; then
        echo "Missing value for --institution-lookback-days" >&2
        exit 1
      fi
      MARKET_INSTITUTION_LOOKBACK_DAYS="$2"
      shift 2
      ;;
    -h|--help)
      usage
...
if [ -n "$DIRECT_MARKET_BRIEF" ]; then
  ...
  trap cleanup_market_lock EXIT INT TERM

  GENERATE_MARKET_CMD="$ROOT_DIR/invest_community/tools/company_report/generate_ashare_market_brief.py"
  UPLOAD_CMD="$ROOT_DIR/invest_community/tools/feishu/create_doc_from_md.py"

  echo "直接生成A股今日走势分析: date=$MARKET_DATE message_id=$DIRECT_MESSAGE_ID"
  GENERATE_OUTPUT=$("$VENV_PY" "$GENERATE_MARKET_CMD" \
    --date "$MARKET_DATE" \
    --top-n "$MARKET_TOP_N" \
    --lookback "$MARKET_LOOKBACK" \
    --institution-top-n "$MARKET_INSTITUTION_TOP_N" \
    --institution-lookback-days "$MARKET_INSTITUTION_LOOKBACK_DAYS" \
    --message-id "$DIRECT_MESSAGE_ID")
  printf '%s\n' "$GENERATE_OUTPUT"

  REPORT_PATH=$(printf '%s\n' "$GENERATE_OUTPUT" | sed -n 's/^已生成报告[：:][[:space:]]*//p' | tail -n 1)
  if [ -z "$REPORT_PATH" ] || [ ! -f "$REPORT_PATH" ]; then
    echo "无法解析或找到生成的A股走势分析文件: $REPORT_PATH" >&2
    exit 1
  fi
