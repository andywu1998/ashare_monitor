"""Cycle screener for all stocks based on local MySQL data."""
...
from ashare_monitor.stock_sync.db import (
    fetch_all_stocks_with_last_trade,
    fetch_daily_rows_for_stock,
)
...
def find_uptrend_stocks(
    ...
    stock_rows = fetch_all_stocks_with_last_trade(mysql_config=mysql_config)
    if max_stocks > 0:
        stock_rows = stock_rows[:max_stocks]

    results: List[UptrendStock] = []
    for ts_code, name, _last_trade in stock_rows:
        raw = fetch_daily_rows_for_stock(
            ts_code,
            start_date,
            real_end,
